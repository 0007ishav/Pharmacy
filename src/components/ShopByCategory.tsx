...
const ShopByCategoryPage: React.FC = () => {
  ...
  return (
    <div className="h-[40rem] w-full dark:bg-black relative flex flex-col overflow-hidden mb-10">
      <hr className="my-4 pb-2" />

      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 px-7">
        <h3 className="mb-2 sm:mb-0 border-b-2 border-gray-300 pb-2  bg-gradient-to-b">
          Shop By Category :-
        </h3>
        <button className="shadow-sm rounded-full px-4 py-2 bg-gray-200 text-gray-800 hover:bg-gray-300 transition duration-200 sm:px-4 sm:py-1">
          See All
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4 lg:gap-14 m-14 relative">
        <button className="shadow-[inset_0_0_0_2px_#616467] px-6 py-2 sm:px-12 sm:py-4 rounded-full tracking-widest uppercase font-bold bg-transparent hover:bg-[#616467] hover:text-white dark:text-neutral-200 flex items-center gap-8 transition duration-200">
          ...
          <div className="w-full">
            <div className="overflow-x-auto flex flex-nowrap w-full">
              {productData.map((product) => (
                <div
                  key={product.id}
                  className="flex-none bg-gray-200 p-4 rounded-lg shadow-md mr-4"
                >
                  <img
                    src={product.image}
                    alt={product.name}
